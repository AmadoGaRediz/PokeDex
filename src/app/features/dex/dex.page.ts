...
@Component({
  ...
  template: `
    <section class="hero">
      <div><p class="eyebrow">MODO POKÉDEX</p><h1>Completa la Pokédex,<br><span>una carta a la vez.</span></h1><p>No importa la expansión, rareza o idioma: cualquier carta cuenta.</p></div>
      <div class="screen"><strong>{{store.percent()}}%</strong><span>{{store.ownedCount()}} obtenidos</span><i></i></div>
  ...
  `,
  styles: [`
    .hero{position:relative;border-radius:34px;background:linear-gradient(135deg,#9d0f18,#e63946 55%,#ffcb05);color:white;padding:42px 44px;display:grid;grid-template-columns:1fr 220px;gap:22px;align-items:center;overflow:hidden;box-shadow:var(--shadow-lg);margin-bottom:20px;border:10px solid color-mix(in srgb,#111 18%,transparent)}.hero:before{content:'';position:absolute;left:18px;top:18px;width:28px;height:28px;border-radius:50%;background:#7ad7ff;box-shadow:38px 0 #ffe46b,72px 0 #43d17a}.eyebrow{font-size:.72rem;letter-spacing:.22em;font-weight:950;color:#ffd65a;margin-top:22px}.hero h1{font-size:clamp(2.2rem,6vw,4.6rem);line-height:.98;margin:8px 0 16px;letter-spacing:-.05em}.hero h1 span{color:#ffd65a}.hero p:last-child{max-width:530px;opacity:.92}.screen{position:relative;z-index:1;min-height:160px;border:8px solid #282c34;border-radius:24px;background:linear-gradient(160deg,#101519,#1e3a32);display:grid;place-items:center;text-align:center;box-shadow:inset 0 0 0 2px #ffffff18}.screen strong{font-size:3rem;color:#7dff9a;text-shadow:0 0 18px #7dff9a66}.screen span{color:#c8ffcf;font-weight:900}.screen i{width:44px;height:9px;border-radius:999px;background:#fa4753;display:block}.hero-ball{position:absolute;right:-70px;bottom:-85px;width:220px;height:220px;border:24px solid #fff3;border-radius:50%;flex:none}.hero-ball:before{content:'';position:absolute;left:-24px;right:-24px;top:calc(50% - 12px);height:24px;background:#fff3}.hero-ball div{width:65px;height:65px;border:18px solid #fff3;border-radius:50%;position:absolute;inset:0;margin:auto;background:#d92835}
    ...
    @media(max-width:850px){.hero{grid-template-columns:1fr}.screen{min-height:120px}.filters{grid-template-columns:1fr 1fr}.search{grid-column:1/-1}.hero-ball{display:none}}@media(max-width:520px){.hero{padding:28px 18px;border-radius:24px;border-width:6px}.screen strong{font-size:2.4rem}.filters{grid-template-columns:1fr;position:sticky;top:130px;z-index:9}.search{grid-column:auto}.grid{grid-template-columns:repeat(2,minmax(0,1fr));gap:10px}}
  `]
})
